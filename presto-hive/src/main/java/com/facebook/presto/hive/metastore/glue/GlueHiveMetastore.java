        ...
        implements ExtendedHiveMetastore
{
    ...
    {
        this(hdfsEnvironment, glueConfig, new GlueClientFactory(glueConfig).newClient());
    }
    ...
    public List<String> getAllDatabases()
    {
        try {
            ...
            do {
                ...
                result.getDatabaseList().stream().forEach(db -> dbNames.add(db.getName()));
            }
    ...
    {
        throw new GlueUnsupportedException(HIVE_METASTORE_ERROR, "getTableColumnStatistics is not supported by Glue, please set hive.table-statistics-enabled=false");
    }
    ...
    {
        throw new GlueUnsupportedException(HIVE_METASTORE_ERROR, "getPartitionColumnStatistics is not supported by Glue, please set hive.table-statistics-enabled=false");
    }
    ...
    public Optional<List<String>> getAllTables(String databaseName)
    {
        try {
            ...
            do {
                ...
                result.getTableList().stream().forEach(tbl -> tblNames.add(tbl.getName()));
                nextToken = result.getNextToken();
            }
            while (nextToken != null);

            return Optional.of(tblNames);
        }
        catch (EntityNotFoundException e) {
        ...
        try {
            ...
            do {
                ...
                result.getTableList().stream()
                        .filter(tbl -> VIRTUAL_VIEW.name().equals(tbl.getTableType()))
                        .forEach(tbl -> views.add(tbl.getName()));
                nextToken = result.getNextToken();
            }
            while (nextToken != null);
    ...
    public void createDatabase(Database database)
    {
        if (!database.getLocation().isPresent() && defaultDir.isPresent()) {
            String dbLocation = new Path(defaultDir.get(), database.getDatabaseName()).toString();
            database = Database.builder(database)
                    .setLocation(Optional.of(dbLocation))
                    .build();
        }

        try {
            DatabaseInput dbInput = GlueInputConverter.convertDatabase(database);
            glueClient.createDatabase(new CreateDatabaseRequest().withDatabaseInput(dbInput));
        }
    ...
    {
        try {
            Database db = getDatabase(databaseName).orElseThrow(() -> new SchemaNotFoundException(databaseName));
            DatabaseInput renamedDb = GlueInputConverter.convertDatabase(db).withName(newDatabaseName);
            glueClient.updateDatabase(new UpdateDatabaseRequest()
                    .withName(databaseName)
                    .withDatabaseInput(renamedDb));
        }
        catch (AmazonServiceException e) {
            throw new PrestoException(HIVE_METASTORE_ERROR, e);
        }
    ...
    {
        Table tbl = getTableOrElseThrow(databaseName, tableName);
        ...
        String tblLocation = tbl.getStorage().getLocation();
        if (deleteData && !isExternalTable(tbl) && !isNullOrEmpty(tblLocation)) {
            deleteDir(hdfsContext, hdfsEnvironment, new Path(tblLocation), true);
        }
    }

    private static boolean isExternalTable(Table tbl)
    {
        return tbl.getTableType().equals(EXTERNAL_TABLE.name());
    }
    ...
    {
        throw new GlueUnsupportedException(HIVE_METASTORE_ERROR, "Table rename is not yet supported by Glue service");
    }
    ...
    /**
     * @param parts List of partition values to filter on which can be full or partial.
        ...
        try {
            ...
            do {
                ...
                result.getPartitions().stream()
                        .forEach(p -> partitions.add(GlueToPrestoConverter.convertPartition(p)));
                nextToken = result.getNextToken();
            }
            while (nextToken != null);
    ...
    private List<String> buildPartitionNames(List<Column> partitionColumns, List<Partition> partitions)
    {
        ...
        List<String> partitionKeys = partitionColumns.stream().map(Column::getName).collect(toList());
        partitions.stream().forEach(p -> partitionNames.add(toPartitionName(partitionKeys, p.getValues())));
        return partitionNames;
    }

    /**
     * @param partitionNames is a list of full partition names
     * ex: partition keys = ['a', 'b']
     * partition names:
     * ['a=1/b=2', 'a=2/b=2']
     * @return is a mapping of partition name to partition object.
     */
    @Override
    ...
    private List<Partition> batchGetPartition(String databaseName, String tableName, List<String> partitionNames)
    {
        try {
            List<PartitionValueList> partitionValueLists = partitionNames.stream()
                    .map(p -> new PartitionValueList().withValues(toPartitionValues(p))).collect(toList());
            ...
            for (Future<BatchGetPartitionResult> future : batchGetPartitionFutures) {
                ...
                batchResult.getPartitions().stream().forEach(p -> result.add(GlueToPrestoConverter.convertPartition(p)));
            }
    ...
    public void dropPartition(String databaseName, String tableName, List<String> parts, boolean deleteData)
    {
        ...
        if (deleteData && !isExternalTable(tbl) && !isNullOrEmpty(partLocation)) {
    ...
    {
        throw new GlueUnsupportedException(HIVE_METASTORE_ERROR, "getDatabasePrivileges is not supported by Glue");
    }

    @Override
    public Set<HivePrivilegeInfo> getTablePrivileges(String user, String databaseName, String tableName)
    {
        throw new GlueUnsupportedException(HIVE_METASTORE_ERROR, "getTablePrivileges is not supported by Glue");
    }

    @Override
    public void grantTablePrivileges(String databaseName, String tableName, String grantee, Set<HivePrivilegeInfo> privileges)
    {
        throw new GlueUnsupportedException(HIVE_METASTORE_ERROR, "grantTablePrivileges is not supported by Glue");
    }

    @Override
    public void revokeTablePrivileges(String databaseName, String tableName, String grantee, Set<HivePrivilegeInfo> privileges)
    {
        throw new GlueUnsupportedException(HIVE_METASTORE_ERROR, "revokeTablePrivileges is not supported by Glue");
    }
}
